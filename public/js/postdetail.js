...
////////////////////////////////////Post Reply//////////////////////////////
///////////////////////////////////////////////////////////////////////////
const post_Reply = document.querySelector(".icon_post_btn_reply_model");
post_Reply.addEventListener("click", () => {
  const form = new FormData();
  const images = document.querySelector("#new_post_imgs");
  form.append("reply", document.querySelector("#new_post_text1").value);
...
menu_bar_PostDetails.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("seeReview")) {
    const url = `http://127.0.0.1:3000/api/v1/post/${postId}/review/getReview`;
    getReview(url);
  }
  ...
  if (e.target.classList.contains("DeletePost")) {
    const url = `http://127.0.0.1:3000/api/v1/post/delete/${postId}`;
...
const activate_like = async function () {
  const promise = await fetch(
    `http://127.0.0.1:3000/api/v1/post/getOnepost/${postid}`
  );
  const res = await promise.json();
